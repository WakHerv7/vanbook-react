...
import { selectAllReceivedPayments,  getReceivedPaymentsStatus, getReceivedPaymentsError, fetchReceivedPayments }from '../../../Reducers/receivedPaymentsSlice';
...
function ReceivePaymentsList(props) {   
    ...
    // --------------------------------------------------------
    const receivedPayments = useSelector(selectAllReceivedPayments);
    const receivedPaymentsStatus = useSelector(getReceivedPaymentsStatus);
    const receivedPaymentsError = useSelector(getReceivedPaymentsError);
    useEffect(() => {
        if (receivedPaymentsStatus === 'idle') {
            dispatch(fetchReceivedPayments())            
        }        
    }, [receivedPaymentsStatus, dispatch])
    ...
    if (receivedPaymentsStatus === 'loading') {
        renderedReceipts = <tr><td>...</td></tr>;
    } else if (receivedPaymentsStatus === 'succeeded') {
        renderedReceipts = Array.isArray(receivedPayments) && receivedPayments.map((receivedPayment, index) => (
            <tr key={index} className="bg-gray-100 border-b">
                ...
                <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {receivedPayment.person.name}
                </td>                            
                <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {receivedPayment.deposit_account.name}
                </td>
                <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {receivedPayment.payment_method.name}
                </td>                
                <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {receivedPayment.total_amount}
                </td>
            </tr>
    ))
    } else if (receivedPaymentsStatus === 'failed') {
        renderedReceipts = {receivedPaymentsError};
    }
            ...
            <div>
                ...
                <div className="flex w-full mt-5 mb-7">
                    <div className="flex flex-col w-full gap-5">
                        <h1 className='text-[30px] myprimarytextcolor'>Received payments</h1>
                        ...
                        <div className="h-fit">
                            <Link to={"/dashboard/new-payment"} className="flex gap-2 outline-none  bg-[#41436a] border-1 border-[#41436a] rounded-md text-white text-sm px-3 py-2">
