...
import { updateReceivedPayment, addNewReceivedPayment, fetchReceivedPaymentById, selectReceivedPaymentById,selectAllReceivedPayments,  getReceivedPaymentsStatus, getReceivedPaymentsError, fetchReceivedPayments }from '../../../../Reducers/receivedPaymentsSlice';
...
import { selectAllInvoices,  getInvoicesStatus, getInvoicesError, fetchInvoices, fetchInvoicesWithOpenBalance }from '../../../../Reducers/invoicesSlice.js';
...
function ReceivePaymentForm(props) {
    ...
    //---------------------------------------------------------
    const myReceivedPayment = useSelector((state) => selectReceivedPaymentById(state, Number(id)));
    const receivedPaymentStatus = useSelector(getReceivedPaymentsStatus);
    const receivedPaymentError = useSelector(getReceivedPaymentsError);
    useEffect(() => {
        if (receivedPaymentStatus === 'idle' && id) {
            dispatch(fetchReceivedPaymentById({ id: Number(id) })).unwrap()          
        }
        else if (receivedPaymentStatus === 'succeeded' && Array.isArray(myReceivedPayment) && myReceivedPayment) {
    ...
    const dispatch_and_resetform = (toSubmit) => {
        if(myReceivedPayment?.id){
            toSubmit['id'] = myReceivedPayment?.id;
            dispatch(
                updateReceivedPayment(toSubmit)
            ).unwrap()
        } else {
            dispatch(
                addNewReceivedPayment(toSubmit)
            ).unwrap()
        }
        ...
        <div className="main_page_container px-10 bg-[#F0F0F0] flex flex-col justify-between">
            ...
            <div>
                    ...
                    <div className="flex flex-col gap-5">
                        ...
                        <div className="flex gap-10">
                            <div className="flex flex-col gap-1">
                                <label className='myprimarytextcolor'>Received from</label>
                                {myPersons?                                
                                <SelectSearch
                ...
                <section className="">
                    ...
                    {myPaymentMethods && myAccounts && myPersons && myInvoices ?
                        <>
                    {[OutstandingTransactionsTable({myPaymentMethods, receiptsAndPayments:myInvoices,
