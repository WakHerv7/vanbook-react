import {React, useState, useEffect }from 'react';
import { Link, useNavigate } from 'react-router-dom';
import {FiChevronLeft, FiPlus } from "react-icons/fi";
import { AiOutlineEdit } from "react-icons/ai";
import { BsThreeDots } from "react-icons/bs";
import { RiDeleteBinLine } from "react-icons/ri";

import AddSchoolPaymentConfigModal from './AddSchoolPaymentConfig/AddSchoolPaymentConfigModal.jsx';
import EditSchoolPaymentConfigModal from './EditSchoolPaymentConfig/EditSchoolPaymentConfigModal.jsx';
import DeleteSchoolPaymentConfigModal from './DeleteSchoolPaymentConfig/DeleteSchoolPaymentConfigModal.jsx';

import { useSelector, useDispatch }from 'react-redux';
import { selectAllSchoolPaymentConfigs,  getSchoolPaymentConfigsStatus, getSchoolPaymentConfigsError, fetchSchoolPaymentConfigs }from '../../../Reducers/schoolPaymentConfigsSlice';
import { selectAllAccounts,  getAccountsStatus, getAccountsError, fetchAccounts }from '../../../Reducers/accountsSlice';


const listSchoolPaymentConfigs = [
    {value:"SchoolPaymentConfig",text:"Element"},
    {value:"SchoolPaymentConfig",text:"Element"},
    {value:"SchoolPaymentConfig",text:"Element"},
    {value:"SchoolPaymentConfig",text:"Element"}
];

function SchoolPaymentConfigsList(props) {        
    const dispatch = useDispatch();
    const navigate = useNavigate();

    const [modalOpen, setModalOpen] = useState(false);    
    const [editModalOpen, setEditModalOpen] = useState(false);
    const [deleteModalOpen, setDeleteModalOpen] = useState(false);
    const [idToEdit, setIdToEdit] = useState(null);
    const [idToDelete, setIdToDelete] = useState(null);
    
    const schoolPaymentConfigs = useSelector(selectAllSchoolPaymentConfigs);
    const schoolPaymentConfigsStatus = useSelector(getSchoolPaymentConfigsStatus);
    const schoolPaymentConfigsError = useSelector(getSchoolPaymentConfigsError);
    useEffect(() => {
        if (schoolPaymentConfigsStatus === 'idle') {
            dispatch(fetchSchoolPaymentConfigs())            
        }
    }, [schoolPaymentConfigsStatus, dispatch])

    // const accounts = useSelector(selectAllAccounts);
    // const accountsStatus = useSelector(getAccountsStatus);
    // const accountsError = useSelector(getAccountsError);
    // useEffect(() => {
    //     if (accountsStatus === 'idle') {
    //         dispatch(fetchAccounts())            
    //     }
    // }, [accountsStatus, dispatch]) 

    const handleModalOpen = () => {   
        modalOpen ? setModalOpen(false) : setModalOpen(true)   
    }
    const handleEditModalOpen = (id) => {  
        if (editModalOpen) {
            setIdToEdit(null)
            setEditModalOpen(false)
        }
        else {
            
            setIdToEdit(id)
            setEditModalOpen(true)
        }
    }

    const handleDeleteModalOpen = (id) => {  
        if (deleteModalOpen) {
            setIdToDelete(null)
            setDeleteModalOpen(false)
        }
        else {            
            setIdToDelete(id)
            setDeleteModalOpen(true)
        }
    }



    let renderedSchoolPaymentConfigs;
    if (schoolPaymentConfigsStatus === 'loading') {
        renderedSchoolPaymentConfigs = <tr><td>...</td></tr>;
    } else if (schoolPaymentConfigsStatus === 'succeeded') {
        renderedSchoolPaymentConfigs = schoolPaymentConfigs.map((schoolPaymentConfig, index) => (
        <tr key={index} className="table_row_w border border-b-slate-300">
            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{index}</td>
            <td className=" text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                {schoolPaymentConfig.name}
            </td>
            <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                {schoolPaymentConfig.classes.map((c, i)=> (
                    <span key={i}>{i>0? ', ':''}{c.name}</span>
                ))}
            </td>
            <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                {schoolPaymentConfig.registration_fees_old_student}
            </td>
            <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                {schoolPaymentConfig.registration_fees_new_student}
            </td>
            <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                {schoolPaymentConfig.school_fees_old_student}
            </td>
            <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                {schoolPaymentConfig.school_fees_new_student}
            </td>
            <td className="flex gap-5 text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                
                <div className="hover:cursor-pointer" onClick={()=>handleEditModalOpen(schoolPaymentConfig.id)}>
                    <AiOutlineEdit size={18} color={"#41436a"}/>
                </div>
                <div className="hover:cursor-pointer" onClick={()=>handleDeleteModalOpen(schoolPaymentConfig.id)}>
                    <RiDeleteBinLine size={18} color={"#41436a"}/>
                </div>
            </td>
        </tr>
    ))
    } else if (schoolPaymentConfigsStatus === 'failed') {
        renderedSchoolPaymentConfigs = <tr><td>{schoolPaymentConfigsError}</td></tr>;
    }

    


    return (
        <>
        <div className="main_page_container bg-[#F0F0F0] flex flex-col justify-between">
            <div>
                <div className="flex px-10 pt-3 justify-between gap-10">
                    <div className="myprimarytextcolor text-xl font-bold">School Payment Configurations</div>
                </div>
                <div className="flex items-center px-10 py-3 justify-between gap-10  border border-b-slate-300">
                    <Link to={""} onClick={() => navigate(-1)} className="flex gap-1 items-center">
                        <FiChevronLeft size={20} color={"#white"}/>
                        <span className="myprimarytextcolor">Back</span>
                    </Link>

                    {/* <div className="select_container flex gap-5 schoolPaymentConfigs-center">
                        <span className="myprimarytextcolor">Look for</span>

                        <select className={`commonSelectInput outline-none h-[40px] px-2  rounded-xl`} name={""} defaultValue={'dflt'}>
                            <option disabled value="dflt">{`Select from the list`}</option>        
                            {
                                listSchoolPaymentConfigs.map((val, ind) => {
                                    return <option key={ind} value={val.value}>{val.text}</option>
                                })                                
                            }
                        </select>

                    </div>

                    <div className="select_container flex gap-5 schoolPaymentConfigs-center">
                        <span className="myprimarytextcolor">In</span>

                        <select className={`commonSelectInput outline-none h-[40px] px-2  rounded-xl`} name={""}  defaultValue={'dflt'}>
                            <option disabled value="dflt">{`Select from the list`}</option>        
                            {
                                listSchoolPaymentConfigs.map((val, ind) => {
                                    return <option key={ind} value={val.value}>{val.text}</option>
                                })                                
                            }
                        </select>

                    </div>

                    <div className="select_container flex gap-5 schoolPaymentConfigs-center">
                        <span className="myprimarytextcolor">Currency</span>

                        <select className={`commonSelectInput outline-none h-[40px] px-2  rounded-xl`} name={""} defaultValue={'dflt'}>
                            <option disabled value="dflt">{`Select from the list`}</option>        
                            {
                                listSchoolPaymentConfigs.map((val, ind) => {
                                    return <option key={ind} value={val.value}>{val.text}</option>
                                })                                
                            }
                        </select>
                    </div> */}

                    <Link to={"#"} onClick={handleModalOpen} className="outline-none flex gap-1 items-center bg-[#41436a] border-1 border-[#41436a] rounded-md text-white text-sm px-3 py-2">
                        <FiPlus size={20} color={"#white"}/>
                        New Config
                    </Link>
                </div>

                <div className="table_container">
                    <table className="w-full">
                    <thead className="bg-white border-b">
                        <tr>
                            <th rowSpan="2" scope="col" className="text-md border-r font-medium myprimarytextcolor px-6 py-4 text-left">
                                #
                            </th>
                            <th rowSpan="2" scope="col" className="text-md border-r font-medium myprimarytextcolor px-6 py-4 text-left">
                                Name   
                            </th>
                            <th rowSpan="2" scope="col" className="text-md border-r font-medium myprimarytextcolor px-6 py-4 text-left">
                                Classes
                            </th>
                            <th colSpan="2"  scope="col" className="text-md border-r border-b font-medium myprimarytextcolor px-6 py-4 text-left">
                                Registration fees
                            </th>
                            <th colSpan="2" scope="col" className="text-md border-b font-medium myprimarytextcolor px-6 py-4 text-left">
                                Training fees
                            </th>
                            <th rowSpan="2" scope="col" className="text-sm border-l font-medium myprimarytextcolor px-6 py-4 text-left">                            
                            </th>
                        </tr>
                        <tr>
                            <th scope="col" className="text-sm border-r font-medium myprimarytextcolor px-6 py-4 text-left">
                                Old Student
                            </th>
                            <th scope="col" className="text-sm border-r font-medium myprimarytextcolor px-6 py-4 text-left">
                                New Student
                            </th>
                            <th scope="col" className="text-sm border-r font-medium myprimarytextcolor px-6 py-4 text-left">
                                Old Student
                            </th>
                            <th scope="col" className="text-sm border-r font-medium myprimarytextcolor px-6 py-4 text-left">
                                New Student
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        {renderedSchoolPaymentConfigs}
                    </tbody>
                    </table>
                </div>
            </div>

            <div className="flex px-5 py-3 gap-10 ">

                {/* <div className="select_container flex gap-5 schoolPaymentConfigs-center">
                    <select className={`commonSelectInput outline-none h-[40px] px-2 min-w-[150px] rounded-xl`} name={""}  defaultValue={'dflt'}>
                        <option disabled value="dflt">{`SchoolPaymentConfigs`}</option>        
                        {
                            listSchoolPaymentConfigs.map((val, ind) => {
                                return <option key={ind} value={val.value}>{val.text}</option>
                            })                                
                        }
                    </select>
                </div>
                <div className="select_container flex gap-5 schoolPaymentConfigs-center">
                    <select className={`commonSelectInput outline-none h-[40px] px-2 min-w-[150px] rounded-xl`} name={""}  defaultValue={'dflt'}>
                        <option disabled value="dflt">{`Activites`}</option>        
                        {
                            listSchoolPaymentConfigs.map((val, ind) => {
                                return <option key={ind} value={val.value}>{val.text}</option>
                            })                                
                        }
                    </select>
                </div>
                <div className="select_container flex gap-5 schoolPaymentConfigs-center">
                    <select className={`commonSelectInput outline-none h-[40px] px-2 min-w-[150px] rounded-xl`} name={""}  defaultValue={'dflt'}>
                        <option disabled value="dflt">{`Report`}</option>        
                        {
                            listSchoolPaymentConfigs.map((val, ind) => {
                                return <option key={ind} value={val.value}>{val.text}</option>
                            })                                
                        }
                    </select>
                </div>
                <div className="select_container flex gap-5 schoolPaymentConfigs-center">
                    <select className={`commonSelectInput outline-none h-[40px] px-2 min-w-[150px] rounded-xl`} name={""}  defaultValue={'dflt'}>
                        <option disabled value="dflt">{`Export`}</option>        
                        {
                            listSchoolPaymentConfigs.map((val, ind) => {
                                return <option key={ind} value={val.value}>{val.text}</option>
                            })                                
                        }
                    </select>
                </div> */}

            </div>

        </div>

        {modalOpen ?
        <>
            <AddSchoolPaymentConfigModal 
            handleModalOpen={handleModalOpen}
            modalOpen={modalOpen}
            />
            
        </>
        :
        <></>}
        {editModalOpen ?
        <>
            <EditSchoolPaymentConfigModal 
            handleModalOpen={handleEditModalOpen}
            modalOpen={editModalOpen}
            schoolPaymentConfigId ={idToEdit}
            />
            
        </>
        :
        <></>}
        {deleteModalOpen ?
        <>
            <DeleteSchoolPaymentConfigModal 
            handleDeleteModalOpen={handleDeleteModalOpen}
            deleteModalOpen={deleteModalOpen}
            schoolPaymentConfigId ={idToDelete}
            />
            
        </>
        :
        <></>}
        </>
    );
}

export default SchoolPaymentConfigsList;