...
import OneReceiptModal from './OneReceiptModal.jsx';
import { selectAllReceipts,  getReceiptsStatus, getReceiptsError, fetchReceipts }from '../../../Reducers/receiptsSlice.js';
...
function ReceiptsList(props) {    
    ...
    useEffect(() => {
        if (receiptsStatus === 'idle') {
            ...
            console.log("======================")
            console.log("myReceipts:",myReceipts)
            console.log("======================")

            let total = 0;
            Array.isArray(myReceipts) && myReceipts.map((receipt, index) =>{
                total += parseInt(receipt.total_amount)
            })
    ...
    } else if (receiptsStatus === 'succeeded' && Array.isArray(myReceipts)) {
        renderedReceipts = myReceipts.slice(0).reverse().map((receipt, index) => (
        <tr key={index} onClick={()=>handleModalOpen(receipt.id)} className="table_row_w cursor-pointer">
            <td className="text-gray-900  cursor-pointer font-light px-6 py-4 whitespace-nowrap">
                {receipt.createdAt.split("T")[0]}
            </td>
        ...
        <div className="main_page_container px-10 bg-[#F0F0F0] flex flex-col justify-between">
            ...
            <div>
                ...
                <div className="flex flex-col  gap-5 mt-5 mb-7">
                    <h1 className='text-[30px] myprimarytextcolor'>Receipts</h1>
                    <div className="flex justify-between gap-5">
                        ...
                        <div className="h-fit">
                            <Link to={"/dashboard/new-receipt"} className="flex gap-2 outline-none  bg-[#41436a] border-1 border-[#41436a] rounded-md text-white text-sm px-3 py-2">
