...
import { selectAllPaidBills,  getPaidBillsStatus, getPaidBillsError, fetchPaidBills }from '../../../Reducers/paidBillsSlice';
...
function PaidBillsList(props) {   
    ...
    // --------------------------------------------------------
    const paidBills = useSelector(selectAllPaidBills);
    const paidBillsStatus = useSelector(getPaidBillsStatus);
    const paidBillsError = useSelector(getPaidBillsError);
    useEffect(() => {
        if (paidBillsStatus === 'idle') {
            dispatch(fetchPaidBills())            
        }        
    }, [paidBillsStatus, dispatch])
    ...
    if (paidBillsStatus === 'loading') {
        renderedReceipts = <tr><td>...</td></tr>;
    } else if (paidBillsStatus === 'succeeded') {
        renderedReceipts = Array.isArray(paidBills) && paidBills.map((paidBill, index) => (
            <tr key={index} className="bg-gray-100 border-b">
                
                <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {paidBill.payment_date}
                </td>
                <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {paidBill.number}
                </td>
                <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {paidBill.person.name}
                </td>                            
                <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {paidBill.deposit_account.name}
                </td>
                <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {paidBill.payment_method.name}
                </td>                
                <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                    {paidBill.total_amount}
                </td>
            </tr>
    ))
    } else if (paidBillsStatus === 'failed') {
        renderedReceipts = {paidBillsError};
    }
            ...
            <div>
                ...
                <div className="flex w-full mt-5 mb-7">
                    <div className="flex flex-col w-full gap-5">
                        <h1 className='text-[30px] myprimarytextcolor'>Paid bills</h1>
