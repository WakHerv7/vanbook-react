...
import AddPersonRoleModal from './AddPersonRole/AddPersonRoleModal.jsx';
import EditPersonRoleModal from './EditPersonRole/EditPersonRoleModal.jsx';
import DeletePersonRoleModal from './DeletePersonRole/DeletePersonRoleModal.jsx';
...
const listPersonRoles = [
    {value:"PersonRole",text:"Element"},
    {value:"PersonRole",text:"Element"},
    {value:"PersonRole",text:"Element"},
    {value:"PersonRole",text:"Element"}
];

function PersonRolesList(props) {        
    ...
    let renderedPersonRoles;
    if (personRolesStatus === 'loading') {
        renderedPersonRoles = <tr><td>...</td></tr>;
    } else if (personRolesStatus === 'succeeded') {
        renderedPersonRoles = personRoles.map((personRole, index) => (
        <tr key={index} className="table_row_w border border-b-slate-300">
            ...
            <td className="flex gap-5 text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap">
                ...
                <div className="hover:cursor-pointer" onClick={()=>handleEditModalOpen(personRole.id)}>
                    <AiOutlineEdit size={18} color={"#41436a"}/>
                </div>
                <div className="hover:cursor-pointer" onClick={()=>handleDeleteModalOpen(personRole.id)}>
                    <RiDeleteBinLine size={18} color={"#41436a"}/>
                </div>
            </td>
        </tr>
    ))
    } else if (personRolesStatus === 'failed') {
        renderedPersonRoles = <tr><td>{personRolesError}</td></tr>;
    }
    ...
    return (
        ...
        <div className="main_page_container bg-[#F0F0F0] flex flex-col justify-between">
            <div>
                <div className="flex px-10 pt-3 justify-between gap-10">
                    <div className="myprimarytextcolor text-xl font-bold">Person Roles</div>
                </div>
                <div className="flex items-center px-10 py-3 justify-between gap-10  border border-b-slate-300">
                    ...
                    {/* <div className="select_container flex gap-5 personRoles-center">
            ...
            <div className="flex px-5 py-3 gap-10 ">
                ...
                {/* <div className="select_container flex gap-5 personRoles-center">
                    <select className={`commonSelectInput outline-none h-[40px] px-2 min-w-[150px] rounded-xl`} name={""}  defaultValue={'dflt'}>
                        <option disabled value="dflt">{`PersonRoles`}</option>        
                        {
                            listPersonRoles.map((val, ind) => {
                                return <option key={ind} value={val.value}>{val.text}</option>
                            })                                
        ...
        {deleteModalOpen ?
        <>
            <DeletePersonRoleModal 
            handleDeleteModalOpen={handleDeleteModalOpen}
            deleteModalOpen={deleteModalOpen}
            personRoleId ={idToDelete}
            />
...
export default PersonRolesList;