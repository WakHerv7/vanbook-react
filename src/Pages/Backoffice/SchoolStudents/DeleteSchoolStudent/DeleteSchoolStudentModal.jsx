...
import { selectSchoolStudentById, deleteSchoolStudent } from "../../../../Reducers/schoolStudentsSlice";
import { useSelector }from 'react-redux';



function DeleteAccountTypeModal({schoolStudentId, handleDeleteModalOpen, deleteModalOpen}) {
    ...
    const onDeleteClicked = () => {
        try {
            ...
            dispatch(deleteSchoolStudent({ id: Number(schoolStudentId) })).unwrap()
        } catch (err) {
            ...
            <div className={`modal_container_wh ${deleteModalOpen ? '': 'displayNone'}`}>
                <div className={`modal_content_box_w px-5 py-3`}>
                    <div className="flex flex-col items-center w-full justify-center pt-2">
                        <h1 className={`text-[24px] font-bold myprimarytextcolor`}>Delete Account "{schoolStudentName}"</h1>
                    </div>
