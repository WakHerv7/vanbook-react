...
import { addNewSchoolStudent, selectAllSchoolStudents,  getSchoolStudentsStatus, getSchoolStudentsError, fetchSchoolStudents }from '../../../../Reducers/schoolStudentsSlice';


function AddSchoolStudentModal({handleModalOpen, modalOpen}) {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    
    const [issubschoolStudentof, setIssubschoolStudentof] = useState();
    const [schoolStudentName, setSchoolStudentName] = useState();
    const [schoolStudentParent, setSchoolStudentParent] = useState();
                        ...
                        <div className="flex mt-3 gap-10 w-full">
                            <div className="flex flex-col gap-5 w-full     014">
                                <div className="flex flex-col gap-1">
                                    ...
                                    <input type="text" onChange={(e)=>setSchoolStudentName(e.target.value)} name="schoolStudent_name" id="receiptNumInputId" className="outline-none py-2 px-2 rounded-md border border-[#41436a]" placeholder='...'/>
                                </div>
                                <div className="flex flex-col gap-2">
                                    <label htmlFor="schoolStudent_category_1" className="myprimarytextcolor container_checkbo_select_w mb0w">
                                        Is subclass of
                                        <input type="checkbox" onChange={(e)=>setIssubschoolStudentof(e.target.checked)} name="subschoolStudent" id="schoolStudent_category_1"/>
                                        <span className="checkmark_checkbo_select_w"></span>
                                    </label>
                                    {issubschoolStudentof?                                    
                                    <select onChange={(e)=>(setSchoolStudentParent(e.target.value))} className={`commonSelectInput border border-[#41436a] outline-none h-[40px] px-2 rounded-md`} name={""} defaultValue={'dflt'}>
...
export default AddSchoolStudentModal;