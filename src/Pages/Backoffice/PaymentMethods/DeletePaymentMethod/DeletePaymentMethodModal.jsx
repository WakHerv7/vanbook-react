...
import { selectPaymentMethodById, deletePaymentMethod } from "../../../../Reducers/paymentMethodsSlice";
import { useSelector }from 'react-redux';



function DeleteAccountTypeModal({paymentMethodId, handleDeleteModalOpen, deleteModalOpen}) {
    
    const dispatch = useDispatch();
    const myPaymentMethod = useSelector((state) => selectPaymentMethodById(state, Number(paymentMethodId)));
    const [paymentMethodName, setPaymentMethodName] = useState(myPaymentMethod?.name);
    const [requestStatus, setRequestStatus] = useState('idle');
    
    const onDeleteClicked = () => {
        try {
            setRequestStatus('pending')
            dispatch(deletePaymentMethod({ id: Number(paymentMethodId) })).unwrap()
        } catch (err) {
            ...
            <div className={`modal_container_wh ${deleteModalOpen ? '': 'displayNone'}`}>
                <div className={`modal_content_box_w px-5 py-3`}>
                    <div className="flex flex-col items-center w-full justify-center pt-2">
                        <h1 className={`text-[24px] font-bold myprimarytextcolor`}>Delete Role "{paymentMethodName}"</h1>
                    </div>
