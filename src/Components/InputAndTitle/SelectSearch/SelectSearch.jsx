...
function SelectSearch({itemPlaceholder, selectedItem, itemsList, handleSelected, index=null, itemHasRole=false}) {
    ...
    useEffect(() => {
        if (selectedItem) {
            selectedName.current.innerHTML = selectedItem?.name;
            // setActive(!active)
    ...
    const updateSelected = (item) => {
        selectedName.current.innerHTML = item.name;
        setActive(!active)
        handleSelected(item)
    };
    ...
    return (
        <>
            <div ref={menuRef} className="select-box">
                ...
                <div className={`select_search_dropdown_container_w ${active ? 'active':''}`}>
                    <div  ref={optionsContainer} className={`options-container ${active ? 'active':''}`}>
